...
function (s) {
    function i() {
        ...
        }, l.$body.addClass(s[e].support.touch ? o.touch : o.desktop), l.$wndw.on(r.scroll, function (s) {
        ...
        }), !l.viewportScale && s[e].support.touch && "undefined" != typeof FlameViewportScale && (l.viewportScale = new FlameViewportScale, a.setViewportScale(), l.$wndw.on(r.orientationchange + " " + r.resize, function () {
        ...
        s[e] = function (s, i, e) {
            ...
            _init: function () {
                ...
                this._complementOptions(), this.vars.fixed = "window" == this.opts.wrapper.target, this.nodes.$wrpr = s('<div class="' + o.wrapper + '" />'), this.nodes.$sldr = s('<div class="' + o.slider + '" />').appendTo(this.nodes.$wrpr), this.nodes.$wrpr.addClass(this.vars.fixed ? o.fixed : o.inline).addClass(o("fx-" + this.opts.effect)).addClass(o(this.opts.slides.scale)).addClass(this.opts.wrapper.classes), this.nodes.$wrpr.on(r.open + " " + r.close + " " + r.prev + " " + r.next + " " + r.slideTo, function (s) {
                    ...
                    s.stopPropagation(), i.nodes.$wrpr.toggleClass(o.hover)
            ...
            slideTo: function (i, t) {
                ...
                if (this.slides.index = i, this._loadContents(), n) {
                    ...
                    this.slides.widthPercentage && (h += "%"), t && (this.nodes.$sldr.addClass(o.noanimation), a.transitionend(this.nodes.$sldr, function () {
                        s(this).removeClass(o.noanimation)
                    }, 5));
                    for (var p in s[e].effects) if (p == this.opts.effect) {
                        s[e].effects[p].call(this, h, t);
                        break
                    }
            ...
            _initSlidesFromContent: function () {
                ...
                this.$node.children().each(function () {
                    ...
                    for (var n in s[e].media) if (s[e].media[n].filterSlides.call(i, t)) {
            ...
            _loadContent: function (i, t) {
                ...
                this.nodes.$slides.slice(i, t).each(function () {
                    ...
                    if (0 == i.children().length) {
                        var t = i.data(d.anchor).attr("href");
                        for (var a in s[e].media) if (s[e].media[a].filterAnchors.call(n, t)) {
            ...
            _complementOptions: function () {
                if ("undefined" == typeof this.opts.wrapper.target && (this.opts.wrapper.target = this.$node.is("a") ? "window" : this.$node), "window" != this.opts.wrapper.target && "string" == typeof this.opts.wrapper.target && (this.opts.wrapper.target = s(this.opts.wrapper.target)), this.opts.show = a.complBoolean(this.opts.show, "window" != this.opts.wrapper.target), s.isNumeric(this.opts.slides.width)) this.slides.widthPercentage = !1, this.opts.slides.visible = a.complNumber(this.opts.slides.visible, 1);
                else {
                    var i = a.isPercentage(this.opts.slides.width) ? a.getPercentage(this.opts.slides.width) : !1;
                    this.slides.widthPercentage = !0, this.opts.slides.visible = a.complNumber(this.opts.slides.visible, i ? Math.floor(100 / i) : 1), this.opts.slides.width = i ? i : Math.ceil(1e4 / this.opts.slides.visible) / 100
                }
                this.opts.slides.slide = a.complNumber(this.opts.slides.slide, this.opts.slides.visible), this.opts.slides.offset = a.isPercentage(this.opts.slides.offset) ? a.getPercentage(this.opts.slides.offset) : a.complNumber(this.opts.slides.offset, 0)
            }
...
!
function (t) {
    var a, i, s, e, n, d = "tosrus",
        r = "caption",
        o = !1;
    t[d].prototype["_addon_" + r] = function () {
        o || (a = t[d]._c, i = t[d]._d, s = t[d]._e, e = t[d]._f, n = t[d]._g, a.add("caption uibg"), i.add("caption"), o = !0);
        var p = this,
            c = this.opts[r];
        t.isArray(c) && t[d].deprecated('An array for the option "caption"', 'the option "caption.attributes"'), c.add && this.vars.fixed && ("string" == typeof c.target && (c.target = t(c.target)), c.target instanceof t ? this.nodes.$capt = c.target : (this.nodes.$capt = t('<div class="' + a.caption + '" />').appendTo(this.nodes.$wrpr), this.nodes.$uibg || (this.nodes.$uibg = t('<div class="' + a.uibg + '" />').prependTo(this.nodes.$wrpr))), c.attributes = c.attributes || [], this.nodes.$anchors.each(function () {
            var a = t(this),
                s = a.data(i.slide);
            s.data(i.caption, "");
            for (var e = 0, n = c.attributes.length; n > e; e++) {
            ...
            p.nodes.$capt.text(t)[t.length > 0 ? "removeClass" : "addClass"](a.disabled)
        }))
    }, t[d].defaults[r] = {
        add: !1,
        target: null,
...
function (e) {
    if (e.fn.hammer) {
        var t, s, r, n, i, a = "tosrus",
            o = "drag",
            d = !1;
        e[a].prototype["_addon_" + o] = function () {
            d || (t = e[a]._c, s = e[a]._d, r = e[a]._e, n = e[a]._f, i = e[a]._g, r.add("dragstart dragend dragleft dragright swipeleft swiperight"), d = !0);
            var l = this;
            if (this.opts[o] && "slide" == this.opts.effect && this.nodes.$slides.length > 1) {
                var f = 0,
                    g = !1,
                    u = !1;
                this.nodes.$wrpr.hammer().on(r.dragstart, function (e) {
                    ...
                    if (e.stopPropagation(), e.gesture) {
                        if (e.gesture.preventDefault(), l.nodes.$sldr.removeClass(t.noanimation).addClass(t.fastanimation), n.transitionend(l.nodes.$sldr, function () {
                            l.nodes.$sldr.removeClass(t.fastanimation)
                        }, l.conf.transitionDuration / 2), l.nodes.$sldr.css("margin-left", 0), "left" == g || "right" == g) {
                            if (u) var s = l.slides.visible;
                            else var i = l.nodes.$slides.first().width(),
                                s = Math.floor((Math.abs(f) + i / 2) / i);
                            s > 0 && l.nodes.$wrpr.trigger(r["left" == g ? "next" : "prev"], [s])
                        }
                        g = !1
                    }
                })
...
!
function (e) {
    var t, o, s, n, r, a = "tosrus",
        p = "keys",
        d = !1;
    e[a].prototype["_addon_" + p] = function () {
        d || (t = e[a]._c, o = e[a]._d, s = e[a]._e, n = e[a]._f, r = e[a]._g, s.add("keyup"), d = !0);
        var c = this,
            i = this.opts[p];
...
(function ($) {
    ...
    $[_PLUGIN_].prototype['_addon_' + _ADDON_] = function () {
        ...
        if (pagr.add) {
            ...
            this.nodes.$wrpr.on(_e.sliding, function (e, slide, direct) {
    ...
    $[_PLUGIN_].prototype.updatePagination = function () {
        if (this.nodes.$pagr) {
            this.nodes.$pagr.children().removeClass(_c.selected).eq(this.slides.index).addClass(_c.selected);
        }
    };
...
function (i) {
    ...
    i[n].media[o] = {
        filterAnchors: function (n) {
            return i.inArray(n.toLowerCase().split(".").pop().split("?")[0], ["jpg", "jpe", "jpeg", "gif", "png"]) > -1
        },
...
function (i) {
    ...
    i[s].media[d] = {
        filterAnchors: function (i) {
            return i.toLowerCase().indexOf("vimeo.com/index.html") > -1
        },
        initAnchors: function (o, e) {
            e = e.split("vimeo.com/index.html")[1].split("?")[0] + "?api=1", i('<iframe src="http://player.vimeo.com/video/' + e + '" frameborder="0" allowfullscreen />').appendTo(o), t.call(this, o)
        },
        filterSlides: function (i) {
            return i.is("iframe") && i.attr("src") ? i.attr("src").toLowerCase().indexOf("vimeo.com/video/index.html") > -1 : !1
        },
...
(function ($) {
    ...
    $[_PLUGIN_].media[_MEDIA_] = {
        ...
        initAnchors: function ($s, href) {
            ...
            if (this.opts[_MEDIA_].imageLink) {
                ...
                $('<a href="' + url + '" class="' + $[_PLUGIN_]._c('play') + '" target="_blank" />').appendTo($s);

                $('<img border="0" />').on($[_PLUGIN_]._e.load, function (e) {
                    e.stopPropagation();
                    $s.removeClass($[_PLUGIN_]._c.loading).trigger($[_PLUGIN_]._e.loaded);
                }).appendTo($s).attr('src', href);
            } else {
                $('<iframe src="http://www.youtube.com/embed/' + href + '" frameborder="0" allowfullscreen />').appendTo($s);
        ...
        filterSlides: function ($s) {
            if ($s.is('iframe') && $s.attr('src')) {
                return ($s.attr('src').toLowerCase().indexOf('youtube.com/embed/index.html') > -1);
            }
            return false;
        },
    ...
    function initVideo($s) {
        ...
        $s.removeClass(_c.loading).trigger(_e.loaded).on(_e.loading, function (e) {
            _f.resizeRatio($v, $s, maxWidth, maxHeight, ratio);
        });

        this.nodes.$wrpr.on(_e.sliding, function (e) {
            commandVideo('pause');
        }).on(_e.closing, function (e) {
            commandVideo('stop');
        });

        _g.$wndw.on(_e.resize, function (e) {
            _f.resizeRatio($v, $s, maxWidth, maxHeight, ratio);
        });
